...
/*   Updated: 2023/03/28 13:33:12 by joestrad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief 			This function moves the enemy on the map
 * 
 * @param g 		A pointer to the main structure of the program
 * @param m_y 		The y coordenate of the map (the column)
 * @param m_x 		The x coordenate of the map (the row)
 */
void	ft_draw_enemy(t_game *g, int m_y, int m_x)
{
	static int	delta_t = 0;

	if (delta_t == 1)
		ft_draw_element(g, g->enemy_i1, m_y, m_x);
	else
		ft_draw_element(g, g->enemy_i0, m_y, m_x);
	if (!delta_t)
		delta_t = 1;
	else
		delta_t = 0;
...

{
	...
	while (map_x < g->map_nr)
	{
		...
		while (map_y < g->map_nc)
		{
			...
			else if (g->map[map_x][map_y] == 'B')
				ft_draw_enemy(g, map_y, map_x);
			else
...
void	ft_create_window(t_game *game)
{
	...
	imgs.enemy0 = mlx_load_png("textures/enemy.png");
	imgs.enemy1 = mlx_load_png("textures/enemy1.png");
	...
	game->enemy_i0 = mlx_texture_to_image(game->mlx, imgs.enemy0);
	game->enemy_i1 = mlx_texture_to_image(game->mlx, imgs.enemy1);
	...
	mlx_delete_texture(imgs.enemy0);
	mlx_delete_texture(imgs.enemy1);
}
